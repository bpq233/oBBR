...
ngx_quic_create_datagrams(ngx_connection_t *c)
{
    ...
    if (ngx_current_msec > cg->timer) {
        ...
        if (cg->Acked_s) {
            if (ngx_strcmp(ngx_cong, "BBRv2") == 0) {
                printf("%ld,%ld,%ld,%ld,%.2f,%ld,%ld,%.2f,%d,%ld,%ld\n",ngx_current_msec - cg->start_time, cg->Acked_s, cg->resend_s, cg->send_s, cg->resend_s * 100.0 / ngx_max(cg->send_s,1), cg->resend, cg->send, cg->resend * 100.0 / ngx_max(cg->send,1), cg->bbr2.bw, cg->bbr2.min_rtt, cg->window);
            } else if (ngx_strcmp(ngx_cong, "BBR") == 0
                    || ngx_strcmp(ngx_cong, "BBR-S") == 0
                    || ngx_strcmp(ngx_cong, "oBBR") == 0
                    || ngx_strcmp(ngx_cong, "B3R") == 0) {
                printf("%ld,%ld,%ld,%ld,%.2f,%ld,%ld,%.2f,%d,%ld,%ld\n",ngx_current_msec - cg->start_time, cg->Acked_s, cg->resend_s, cg->send_s, cg->resend_s * 100.0 / ngx_max(cg->send_s,1), cg->resend, cg->send, cg->resend * 100.0 / ngx_max(cg->send,1), cg->bbr.bw, cg->bbr.min_rtt, cg->window);
            } else if (ngx_strcmp(ngx_cong, "CUBIC") == 0) {
                printf("%ld,%ld,%ld,%ld,%.2f,%ld,%ld,%.2f,%ld\n",ngx_current_msec - cg->start_time, cg->Acked_s, cg->resend_s, cg->send_s, cg->resend_s * 100.0 / ngx_max(cg->send_s,1), cg->resend, cg->send, cg->resend * 100.0 / ngx_max(cg->send,1), cg->window);
            }
        }
    ...
    if (cg->in_flight == 0 && cg->sampler.is_app_limited) {
        if (ngx_strcmp(ngx_cong, "BBR") == 0
        || ngx_strcmp(ngx_cong, "BBR-S") == 0
        || ngx_strcmp(ngx_cong, "oBBR") == 0
        || ngx_strcmp(ngx_cong, "B3R") == 0) {
            ngx_bbr_restart_from_idle(&cg->bbr, cg->delivered);
        }
        else if (ngx_strcmp(ngx_cong, "BBRv2") == 0) {
            ngx_bbr2_restart_from_idle(&cg->bbr2, cg->delivered);
        }
    }
