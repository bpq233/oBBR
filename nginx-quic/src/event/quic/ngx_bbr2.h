#ifndef _ngx_BBR2_H_INCLUDED_
#define _ngx_BBR2_H_INCLUDED_
...
typedef struct ngx_bbr2_s {
    ...
    ngx_bbr2_ack_phase  ack_phase;
    /* losses in STARTUP round */
    uint8_t             loss_events_in_round;
    uint64_t            probe_rtt_min_ms;
    uint64_t            probe_rtt_min_ms_stamp;

    /* adjust cwnd in loss recovery */
    ngx_msec_t              recovery_start_time;
    bool                    packet_conservation;
    ngx_msec_t              loss_start_time;
    ngx_bbr2_recovery_mode  recovery_mode;
...
} ngx_bbr2_pacing_gain_phase;

typedef struct ngx_bbr2_context_s {
    uint32_t sample_bw;
