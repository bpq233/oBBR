        ...
        while (*p) {
            ...
            case 'C':
                ...
                if (ngx_strcmp(ngx_cong, "oBBR") == 0) {
                    if (argv[++i]) {
                        char *endptr;
                        BBRU = strtod(argv[i], &endptr);
                        if (BBRU < 0 || BBRU > 1.001) {
                            ngx_log_stderr(0, "invalid option: \"-C oBBR %.2f\"", BBRU);
                            return NGX_ERROR;
                        }
                        goto next;
                    } else {
                        ngx_log_stderr(0, "option \"oBBR\" requires parameter");
                        return NGX_ERROR;
                    }
                } else if (ngx_strcmp(ngx_cong, "BBR") == 0
                    || ngx_strcmp(ngx_cong, "CUBIC") == 0
                    || ngx_strcmp(ngx_cong, "BBR-S") == 0
                    || ngx_strcmp(ngx_cong, "BBRv2") == 0
                    || ngx_strcmp(ngx_cong, "B3R") == 0) {
                    goto next;
                }
                ngx_log_stderr(0, "invalid option: \"-C %s\"", ngx_cong);
                return NGX_ERROR;
            case 'O':
                if (*p) {
                    out_f = (char *) p;
            ...
            case 'l':
                if (*p) {
                    char *endptr;
                    c_gain = strtod((char *) p, &endptr);
                } else if (argv[++i]) {
                    char *endptr;
                    c_gain = strtod(argv[i], &endptr);
                }
                goto next;
